...
from origanization.models import CourseOrg
...
class Courseconfig(object):
    ...
    def queryset(self):
        ...
        qs = super(Courseconfig, self).queryset()
...
class BannerCourseconfig(object):
    '''轮播课程'''
    ...
    def queryset(self):
        #重载queryset方法，来过滤出我们想要的数据的
        qs = super(BannerCourseconfig, self).queryset()
        #只显示is_banner=True的课程
        qs = qs.filter(is_banner=True)
        return qs

class Lessonconfig(object):
    '''章节'''
...
class CourseResourceconfig(object):
    '''课程资源'''
...
xadmin.site.register(BannerCourse, BannerCourseconfig)
